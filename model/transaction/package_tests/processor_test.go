...
func TestTransactionProcessorOK(t *testing.T) {
	tests.TestProcessRequests(t, beater.TransactionV1Route, &model.TransformContext{}, backendRequestInfo, map[string]string{})
}

func TestMinimalTransactionProcessorOK(t *testing.T) {
	tests.TestProcessRequests(t, beater.TransactionV1Route, &model.TransformContext{}, backendRequestInfoIgnoreTimestamp, map[string]string{"@timestamp": "-"})
}
...
func TestProcessorFrontendOK(t *testing.T) {
	...
	tests.TestProcessRequests(t, beater.TransactionV1Route, tctx, frontendRequestInfo, map[string]string{"@timestamp": "-"})
}

func BenchmarkBackendProcessor(b *testing.B) {
	tctx := &model.TransformContext{}
	tests.BenchmarkProcessRequests(b, beater.TransactionV1Route, tctx, backendRequestInfo)
	tests.BenchmarkProcessRequests(b, beater.TransactionV1Route, tctx, backendRequestInfoIgnoreTimestamp)
}
...
func BenchmarkFrontendProcessor(b *testing.B) {
	...
	tests.BenchmarkProcessRequests(b, beater.TransactionV1Route, tctx, frontendRequestInfo)
}
