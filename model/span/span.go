...
var processorEntry = common.MapStr{"name": "transaction", "event": "span"}

var cachedSchema = validation.CreateSchema(schema.ModelSchema, "span")
...
func DecodeSpan(input interface{}, err error) (model.Transformable, error) {
	...
	stacktr, err = model.DecodeStacktrace(raw["stacktrace"], decoder.Err)
	if stacktr != nil {
...
func (s *Span) Events(tctx *model.TransformContext) []beat.Event {
	return []beat.Event{beat.Event{
		Fields: common.MapStr{
			"processor":   processorEntry,
			"span":        s.Transform(tctx),
			"transaction": common.MapStr{"id": s.TransactionId},
			"context":     NewSpanContext(tctx).Merge(s.Context),
		},
		Timestamp: s.Timestamp,
	}}
}

func (s *Span) Transform(tctx *model.TransformContext) common.MapStr {
	if s == nil {
		return nil
	}
	tr := common.MapStr{}
