...
func TestPayloadTransform(t *testing.T) {
	...
	events := p.Events(&model.TransformContext{Config: model.TransformConfig{}})
...
func TestInvalidateCache(t *testing.T) {
	...
	conf := model.TransformConfig{SmapMapper: &smapMapper}
	tctx := &model.TransformContext{Config: conf}

	_, transformable, err := DecodePayload(data)
	assert.NoError(t, err)
	transformable[0].Events(tctx)
