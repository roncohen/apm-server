...
func TestCulprit(t *testing.T) {
	...
	tests := []struct {
		...
		{
			event:   Event{Culprit: &c},
			config:  model.TransformConfig{SmapMapper: &mapper},
			culprit: "foo",
...
func TestSourcemapping(t *testing.T) {
	...
	tctx := model.TransformContext{
		Config: model.TransformConfig{SmapMapper: nil},
		Metadata: model.Metadata{
			Service: &m.Service{},
		},
	}
	...
	trWithSmap := event2.Transform(&tctx2)
