...
func TestProcessorFrontendOK(t *testing.T) {
	...
	tests.TestProcessRequests(t, beater.ErrorV1Route, tctx, frontendRequestInfo, map[string]string{})
}
...
func BenchmarkBackendProcessor(b *testing.B) {
	tctx := &model.TransformContext{
		Config: model.TransformConfig{ExcludeFromGrouping: nil},
	}

	tests.BenchmarkProcessRequests(b, beater.ErrorV1Route, tctx, backendRequestInfo)
	tests.BenchmarkProcessRequests(b, beater.ErrorV1Route, tctx, backendRequestInfoIgnoreTimestamp)
}
...
func BenchmarkFrontendProcessor(b *testing.B) {
	...
	tests.BenchmarkProcessRequests(b, beater.ErrorV1Route, tctx, frontendRequestInfo)
}
