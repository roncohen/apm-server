...
var processorEntry = common.MapStr{"name": "error", "event": "error"}

var cachedPayloadSchema = validation.CreateSchema(schema.PayloadSchema, "error")
var cachedModelSchema = validation.CreateSchema(schema.ModelSchema, "error")
...
func (e *Event) Events(tctx *m.TransformContext) []beat.Event {
	ev := beat.Event{
		Fields: common.MapStr{
			"error":     e.Transform(tctx),
			"processor": processorEntry,
			"context":   m.NewContext(tctx).Merge(e.Context),
		},
		Timestamp: e.Timestamp,
	}

	if e.Transaction != nil {
		ev.Fields["transaction"] = common.MapStr{"id": e.Transaction.Id}
	}
	return []beat.Event{ev}
}

func (e *Event) Transform(tctx *m.TransformContext) common.MapStr {
...
func (e *Event) updateCulprit(config m.TransformConfig) {
...
func (e *Event) addException(tctx *m.TransformContext) {
	if e.Exception == nil {
		return
	}
	ex := common.MapStr{}
...
func (e *Event) addLog(tctx *m.TransformContext) {
	if e.Log == nil {
		return
	}
	log := common.MapStr{}
