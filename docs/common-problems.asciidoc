...
This happens when a single event exceeds the limit set in APM Server.
The limit can be increased by increasing the configuration setting `apm-server.max_event_size`.
